...
library ERC721LazyMintLibrary {
    ...
    bytes4 constant public ERC721_LAZY_MINT_ASSET_CLASS = bytes4(keccak256("ERC721_LAZY_MINT_CLASS"));
    ...
    bytes32 public constant ERC721_LAZY_MINT_DATA_TYPEHASH = keccak256(
    ...
    function getHash(ERC721LazyMintData memory erc721LazyMintData) internal pure returns (bytes32) {
        ...
        bytes32[] memory royaltyInfosHashes = new bytes32[](erc721LazyMintData.royaltyInfos.length);
        for (uint256 i = 0; i < erc721LazyMintData.royaltyInfos.length; ++i) {
            royaltyInfosHashes[i] = PartLibrary.getHash(erc721LazyMintData.royaltyInfos[i]);
        }
